...
def find_cli() -> Optional[Path]:
    ...
    ★ npm으로 설치하면 Windows에서 `claude`가 **.CMD 셸 래퍼**로 PATH에 잡힌다.
      claude-agent-sdk는 .cmd/.bat를 인자 주입 위험 때문에 보안상 거부한다
      (2026-08-18 실측: `ProviderError: Refusing to execute batch script`).
      그래서 PATH에서 찾은 게 .cmd/.bat면 진짜 실행 파일을 계속 찾고, 진짜가
      끝내 없을 때만 마지막 수단으로 그 .cmd를 돌려준다.
    """
    ...
    if which and Path(which).suffix.lower() not in (".cmd", ".bat"):
        return Path(which)
    ...
    # claude-agent-sdk 패키지 자체가 들고 오는 실행 파일 — requirements.txt 로
    # pip install만 하면 누구에게나 있다. 팀원 PC에서도 항상 되는 게 이 경로다.
    try:
    ...
    if which:  # 진짜 실행 파일을 끝내 못 찾았으면 .cmd라도 마지막 수단으로
        return Path(which)
    return None
