# -*- coding: utf-8 -*-
"""S4 — perso 30분 한계에 맞춰 자를 지점을 정한다.
...
끊겨 버린다. 대신:
  ...
  1. 몇 조각이 필요한지 먼저 센다 — `ceil(전체길이 / 28분)`. 45분이면 2조각.
     (perso 한계는 30분이지만 상한을 28분으로 잡아 2분을 마진으로 남긴다.)
...
from scripts.common import (CHUNK_MAX_SEC, CUT_SEARCH_HALF_SEC, PERSO_LIMIT_SEC,
...
def main() -> None:
    ap = argparse.ArgumentParser(description="perso 30분 한계에 맞춰 자를 지점 찾기")
    ...
    print(f"전체 {mmss(total)} · perso 한계 {mmss(PERSO_LIMIT_SEC)} · 조각 상한 {mmss(CHUNK_MAX_SEC)}")
