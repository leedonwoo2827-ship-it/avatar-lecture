...
언어는 `--lang`으로 준다. 기본은 러시아어(ru)다. 우즈벡어(uz)로 뒤집히면 플래그만
바꾸면 되지만, **whisper의 우즈벡어 정확도는 러시아어에 한참 못 미친다** — 그때는
02/words.json을 사람이 더 손봐야 한다고 보는 게 맞다.
...
from scripts.common import current_workspace, mmss, need, paths, save_json
from scripts.whisper_util import load_model, transcribe_file


def main() -> None:
    ap = argparse.ArgumentParser(description="강의 오디오 전사 — 단어 타임스탬프")
    ap.add_argument("--lang", default="ru", help="음성 언어 (ru=러시아어 기본, uz, ko, en…)")
    ...
    print(f"       → 120강 × 45분이면 대략 {ratio * 45 * 120 / 60:.0f}시간")
    print(f"       {P.words}")
