...
플랜은 이렇습니다 (2026-09-04 기준).

| 플랜 | 월 값 | 월 크레딧 |
|---|---|---|
| Creator | $29 · 약 3.8만원 | 600 |
| Pro | $49 부터 · 약 6.5만원 | 1,000 부터 — 필요한 만큼 상위 티어를 고릅니다 |
| Business | 첫 시트 $149 · 약 20만원 (팀원 추가는 시트당 $20) | 1,500 |

**Creator 로는 45분 강의 한 편이 안 됩니다** — 분당 16 이면 37분에서 끊깁니다.

### 1분에 얼마인가

플랜 값을 크레딧으로 나누면 나옵니다 (Pro 기준 · 1달러 1,340원):

| | 모델 IV | 모델 V |
|---|---|---|
| **웹 드래그드랍** | 분당 16 크레딧 = **$0.78 · 약 1,050원** | 분당 43.8 크레딧 = **$2.15 · 약 2,900원** |
| **API** | 분당 **$3.00 · 약 4,000원** | — |

45분 강의 한 편이면 웹 IV 로 **약 47,000원**, 전편을 V 로 뽑으면 약 129,000원,
API 로 부르면 약 180,000원입니다.

플랜은 **월 단위**라 실제로 내는 돈은 월 값입니다 — 위 표는 크레딧을 값으로
환산한 것이고, «한 편 더 구우면 얼마가 더 드나» 를 재는 데 씁니다.

Creator 도 크레딧당 값은 Pro 와 거의 같습니다($0.048 대 $0.049). Business 는
크레딧당 두 배($0.099)라 **시트가 필요할 때만** 값이 맞습니다.
