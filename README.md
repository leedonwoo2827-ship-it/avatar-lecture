...
| | 어떻게 | 값 |
|---|---|---|
| **웹 드래그드랍** (기본) | `05\bundleNN\올릴음성.mp3` 를 HeyGen 웹에 올려 렌더하고, 받은 영상을 그 폴더에 되돌려 놓습니다 | 구독 크레딧 — **IV 분당 16 · V 분당 47** |
| API | 화면에서 바로 부릅니다 | **IV 분당 $3.00** (III 는 $1.00) |

웹 구독 쪽 숫자는 실측입니다 (2026-09-03, HeyGen 프로젝트 목록):

| 렌더 | 길이 | 크레딧 | 분당 |
|---|---|---|---|
| `scene01-우즈벡간호-IV` | 1:11 | 19 | 16.0 |
| `scene02-07우즈벡간호-IV` | 7:23 | 119 | 16.1 |
| `scene01-Caroline-V` | 1:11 | 56 | 47.3 |

두 길이에서 같은 값이 나왔으니 크레딧은 **길이에 정비례**합니다.
**모델을 V 로 올리면 3배**입니다 — 45분 한 편이면 IV 약 720, V 약 2,130 크레딧.

### 한 달에 몇 분이 되나

플랜별 월 크레딧 (2026-09-04 기준). 위 분당 값으로 나눠 본 것입니다:

| 플랜 | 월 크레딧 | 모델 IV | 모델 V |
|---|---|---|---|
| Creator | 600 | 약 37분 | 약 13분 |
| Pro | 1,000 부터 (사용량에 따라 선택) | 약 62분 | 약 21분 |
| Business | 1,500 | 약 93분 | 약 32분 |

**Creator 로는 45분 강의 한 편이 안 됩니다** (37분). Pro 면 한 편에 조금 남고,
Business 면 두 편입니다. 120강을 이 길로 다 굽는다면 플랜이 아니라 **묶음을 몇
편씩 나눠 몇 달에 걸칠지**를 먼저 정해야 합니다.

### 섞어 쓰기 — 도입만 V, 본문은 IV

첫 씬은 사람이 처음 보는 자리라 V 로 뽑고, 본문은 IV 로 한 덩어리로 받는 조합이
됩니다. 45분이면 **도입 1분 V(47) + 본문 44분 IV(708) ≈ 755 크레딧** — 전편을
V 로 뽑는 것의 3분의 1입니다.
